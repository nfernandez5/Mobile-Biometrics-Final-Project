...
X_train, y_train = get_images.get_images('../f22-train-dataset')
...
#X_test, y_test = get_images.get_images('../f22-makeup-test-dataset')
...
clf = ORC(mnb()).fit(X_train, y_train)
matching_scores_svm = clf.predict_proba(X_test)

''' Matching and Decision - Classifer 2 '''
clf = ORC(mlp()).fit(X_train, y_train)
matching_scores_mlp = clf.predict_proba(X_test)

''' Fuse scores '''
matching_scores = (matching_scores_svm + matching_scores_mlp) / 2.0
...
performance_plots.performance(gen_scores, imp_scores, 'MNB-MLP-control-score_fusion', 100)
